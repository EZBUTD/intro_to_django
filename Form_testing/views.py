...
def sign1(request):
    if request.method =='POST':
        ...
        if form.is_valid():
            try:
                ID_test=Comment.objects.filter(Identifier=request.POST['Identifier']).values('comments')[0]['comments'] #test if existing comment is there
...
def sign_detail21(request, ID_value):
    try:
